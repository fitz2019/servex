...
type ExecutorResult struct {
    Response *llm.ChatResponse // 最终响应（工具调用循环结束后）
    Messages []llm.Message     // 完整对话历史（含所有工具调用和结果）
...
type StepEvent struct {
    ...
    Response    *llm.ChatResponse // 本轮 LLM 响应
...
result, err := executor.Run(ctx, []llm.Message{
    llm.UserMessage("北京今天天气怎么样？"),
})
