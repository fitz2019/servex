...
type StepEvent struct {
	...
	Response    *ai.ChatResponse // 本轮 LLM 响应（含 Content/ToolCalls）
...
func (e *Executor) Run(ctx context.Context, messages []ai.Message, opts ...ai.CallOption) (*ExecutorResult, error) {
	...
	if len(tools) > 0 {
		opts = append([]ai.CallOption{ai.WithTools(tools...)}, opts...)
	}
	...
	for round := range e.opts.maxRounds {
		...
		for _, tc := range resp.Message.ToolCalls {
			...
			history = append(history, ai.ToolResultMessage(tc.ID, output))
		}
