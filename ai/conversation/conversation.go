...
func (c *Conversation) Chat(ctx context.Context, input string, opts ...ai.CallOption) (*ai.ChatResponse, error) {
	c.memory.Add(ai.UserMessage(input))
	messages := c.buildMessages()

	resp, err := c.model.Generate(ctx, messages, opts...)
	if err != nil {
		// 回滚刚加入的用户消息
		c.memory.Add(ai.UserMessage("")) // 占位，随后 trim 会处理
		return nil, err
	}
...
func (c *Conversation) buildMessages() []ai.Message {
	history := c.memory.Messages()
	if c.systemPrompt == "" {
		return history
	}
	messages := make([]ai.Message, 0, len(history)+1)
	messages = append(messages, ai.SystemMessage(c.systemPrompt))
...
func (r *autoRecordStreamReader) Recv() (ai.StreamChunk, error) {
...
// Response 获取完整响应.
func (r *autoRecordStreamReader) Response() *ai.ChatResponse {
...
// 编译期接口断言.
var _ ai.StreamReader = (*autoRecordStreamReader)(nil)
