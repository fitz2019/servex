...
OpenAI API 适配器，实现 `ai.ChatModel` 和 `ai.EmbeddingModel` 接口。
...
resp, err := client.Generate(ctx, []ai.Message{
    ...
    ai.UserMessage("解释一下 Go 的 goroutine"),
...
```go
reader, err := client.Stream(ctx, []ai.Message{
    ai.UserMessage("写一首关于秋天的诗"),
})
if err != nil {
    return err
}
...
resp, err := client.EmbedTexts(ctx, []string{
    "Go 并发编程",
    "Kubernetes 容器编排",
}, ai.WithModel("text-embedding-3-large"))
...
```go
// DashScope（通义千问）
client := openai.New(os.Getenv("DASHSCOPE_API_KEY"),
    openai.WithBaseURL("https://dashscope.aliyuncs.com/compatible-mode/v1"),
    openai.WithModel("qwen-plus"),
)

// DeepSeek
client := openai.New(os.Getenv("DEEPSEEK_API_KEY"),
