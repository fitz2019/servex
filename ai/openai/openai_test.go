...
import (
	...
	"github.com/Tsukikage7/servex/ai/openai"
)
...
func TestGenerate_Success(t *testing.T) {
	...
	resp, err := client.Generate(t.Context(), []ai.Message{ai.UserMessage("Hi")})
	if err != nil {
...
func TestGenerate_RateLimited(t *testing.T) {
	...
	_, err := client.Generate(t.Context(), []ai.Message{ai.UserMessage("Hi")})
	if err == nil {
		t.Fatal("期望错误，得到 nil")
	}
...
func TestStream_SSEParsing(t *testing.T) {
	...
	client := openai.New("test-key", openai.WithBaseURL(srv.URL))
	reader, err := client.Stream(t.Context(), []ai.Message{ai.UserMessage("Hi")})
	if err != nil {
...
func TestGenerate_WithTools(t *testing.T) {
	...
	resp, err := client.Generate(t.Context(),
		[]ai.Message{ai.UserMessage("北京天气怎么样？")},
		ai.WithTools(tool))
	if err != nil {
		t.Fatalf("Generate 失败: %v", err)
	}
...
func TestGenerate_Integration(t *testing.T) {
	...
	resp, err := client.Generate(t.Context(), []ai.Message{ai.UserMessage("说'你好'")},
		ai.WithMaxTokens(10))
	if err != nil {
