...
// 编译期接口断言.
var (
	_ ai.ChatModel      = (*Client)(nil)
	_ ai.EmbeddingModel = (*Client)(nil)
)
...
// Generate 非流式生成.
func (c *Client) Generate(ctx context.Context, messages []Message, opts ...ai.CallOption) (*ai.ChatResponse, error) {
	...
	// 流式回调（在非流模式下的后处理）
	if o.StreamFunc != nil {
		_ = o.StreamFunc(ctx, ai.StreamChunk{Delta: msg.Content, FinishReason: choice.FinishReason})
	}
...
// Stream 流式生成.
func (c *Client) Stream(ctx context.Context, messages []Message, opts ...ai.CallOption) (ai.StreamReader, error) {
	o := ai.ApplyOptions(opts)
...
func (c *Client) EmbedTexts(ctx context.Context, texts []string, opts ...ai.CallOption) (*ai.EmbedResponse, error) {
	o := ai.ApplyOptions(opts)
...
// Message 是 ai.Message 的类型别名，便于包内使用.
type Message = ai.Message

// buildChatRequest 构建 OpenAI 聊天请求体.
func (c *Client) buildChatRequest(messages []ai.Message, o ai.CallOptions, stream bool) (chatRequest, error) {
	model := c.model
	if o.Model != "" {
		model = o.Model
	}
...
// convertMessage 将 ai.Message 转换为 OpenAI 格式.
func (c *Client) convertMessage(msg ai.Message) (chatMessage, error) {
	cm := chatMessage{
...
// convertResponseMessage 将 OpenAI 响应消息转换为 ai.Message.
func (c *Client) convertResponseMessage(msg chatMessage) ai.Message {
	m := ai.Message{
	...
	// 工具调用
	if len(msg.ToolCalls) > 0 {
		m.ToolCalls = make([]ai.ToolCall, 0, len(msg.ToolCalls))
		for _, tc := range msg.ToolCalls {
...
func (c *Client) parseError(statusCode, retryAfter int, body []byte) error {
	...
	case 400:
		if code == "context_length_exceeded" || errResp.Error.Type == "invalid_request_error" {
			if len(body) > 0 {
				return fmt.Errorf("%w: %w", ai.ErrContextLength, apiErr)
			}
		}
		return apiErr
	default:
