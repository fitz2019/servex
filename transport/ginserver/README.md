...
`ginserver` 包为 [Gin](https://github.com/gin-gonic/gin) 框架提供类型安全的 Handler 适配器。

在完整保留 Gin 原生路由与中间件体系的前提下，统一处理请求解码、参数校验、响应包装与 i18n 错误消息。
...
```go
r.GET("/users/:id", ginserver.HandleWith(
    func(c *gin.Context) (GetUserReq, error) {
...
func main() {
    ...
    r.POST("/users", ginserver.Handle(createUser))
    r.GET("/users/:id", ginserver.HandleWith(decodeGetUser, getUser))
...
func decodeGetUser(c *gin.Context) (GetUserReq, error) {
    return GetUserReq{ID: c.Param("id")}, nil
}
